...
const WriterPublicProfile = () => {
  ...
  React.useEffect(() => {
    let start = async () => {
      setLoading(true);
      let writerBlogsInfo = await axios.get(
        `${BACK_END_URL}/blog/singleWriterBlogs/${writerId}?category=${category}`
      );
      let currentWriterInfo = await axios.get(
        `${BACK_END_URL}/writer/${writerId}`
      );
